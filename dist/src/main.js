...
window.mostrarTela = function (tela) {
    ...
    if (kofiBtn) {
        kofiBtn.style.display = (tela === "game") ? "none" : "flex";
    }
...
window.ehMinhaVez = function () {
    if (!window.estadoJogo.jogadores || window.estadoJogo.jogadores.length === 0)
        return false;
    const idx = window.estadoJogo.jogadores.findIndex((j) => j.nome === window.nomeAtual);
    let resultado = false;
...
// =========================
window.mostrarJogo = function (codigo, jogadores, espectadores) {
    // 1. Switch Screen
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (btnStart) {
        btnStart.onclick = () => {
            ...
            const nomeInput = document.getElementById("nome-criar");
    ...
    if (btnJoin) {
        btnJoin.onclick = () => {
            ...
            const nomeInput = document.getElementById("nome-entrar");
            const nome = nomeInput.value.trim();
            const tipoInput = document.querySelector('input[name="tipo-entrada"]:checked');
    ...
    if (btnMute) {
        ...
        btnMute.onclick = (e) => {
            ...
            if (window.EventBus) {
                window.EventBus.emit('AUDIO:MUTE:CHANGED', { isMuted: newState });
            }
        };
    ...
    if (btnPlay) {
        btnPlay.onclick = function () {
            ...
            if (window.ScreenManager) {
                window.ScreenManager.navigateTo('game-modes');
            }
        };
    }
    ...
    if (btnOptions) {
        btnOptions.onclick = function () {
            ...
            if (window.ScreenManager) {
                window.ScreenManager.navigateTo('settings');
            }
        };
    }
    ...
    if (btnPvE) {
        btnPvE.onclick = function () {
            ...
            if (window.RoomManager) {
                ...
                window.RoomManager.criarSala(codigo); // Mock creation
                window.RoomManager.entrarSala(codigo, "Jogador", "jogador");
                // Init Mode
                if (window.PvEMode) {
                    window.currentGameMode = new window.PvEMode();
                    window.currentGameMode.start({ roomCode: codigo, playerName: "Jogador" });
                }
                // Show Game (Use mostrarJogo to init listeners like CoinFlip)
                if (window.mostrarJogo) {
                    window.mostrarJogo(codigo, [{ nome: "Jogador" }, { nome: "Bot" }], []);
                }
                else if (window.mostrarTela && !window.ScreenManager) {
                    window.mostrarTela("game");
                }
            }
            else {
    ...
    if (btnDesafiar) {
        btnDesafiar.onclick = function () {
            ...
            if (window.currentGameMode && !window.currentGameMode.canPerformAction("BOTAO_DESAFIAR")) {
                return;
            }
    ...
    if (btnBack && onlineMenu && mainMenuBtns && gameModesScreen) {
        ...
        if (btnCreateTab && btnJoinTab && sectionCreate && sectionJoin) {
            ...
            const radio1x1 = document.querySelector('input[name="mode"][value="1x1"]');
            if (radio1x1)
                radio1x1.checked = true;
            // Synchronize Nicknames
            const inputNomeCriar = document.getElementById("nome-criar");
            const inputNomeEntrar = document.getElementById("nome-entrar");
    ...
    window.startBotVsBot = function () {
        ...
        // For now, we just log.
        if (window.Logger)
            window.Logger.game("Bot vs Bot Mode initiated (Simulation Only)");
    };
    // --- GLOBAL TRANSLATION UPDATE FUNCTION ---
    // Function to update all HTML elements with data-i18n attributes
    window.updateHTMLTranslations = function () {
