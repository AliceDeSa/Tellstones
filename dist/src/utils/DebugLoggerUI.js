...
export class DebugLoggerUI {
    ...
    renderContent() {
        ...
        Object.values(LogCategory).forEach(cat => {
            const chk = this.container.querySelector(`#log-cat-${cat}`);
            if (chk) {
                chk.onchange = (e) => {
                    Logger.toggleCategory(cat, e.target.checked);
                };
            }
        });
        // EventBus Test Buttons
        const btnEventBusInfo = this.container.querySelector("#btn-eventbus-info");
        if (btnEventBusInfo) {
            btnEventBusInfo.onclick = () => {
                if (window.EventBus) {
                    const info = window.EventBus.getDebugInfo();
        ...
        const btnEventBusHistory = this.container.querySelector("#btn-eventbus-history");
        ...
        const btnEventBusTestGame = this.container.querySelector("#btn-eventbus-test-game");
        if (btnEventBusTestGame) {
            btnEventBusTestGame.onclick = () => {
                if (window.EventBus) {
                    Logger.info(LogCategory.SYSTEM, "✅ Testando GAME:START event...");
                    window.EventBus.emit('GAME:START', { mode: 'pve' });
                }
                else {
                    Logger.error(LogCategory.SYSTEM, "❌ EventBus não encontrado!");
                }
        ...
        const btnEventBusTestTurn = this.container.querySelector("#btn-eventbus-test-turn");
        if (btnEventBusTestTurn) {
            btnEventBusTestTurn.onclick = () => {
                if (window.EventBus) {
                    Logger.info(LogCategory.SYSTEM, "✅ Testando TURN:START event...");
                    window.EventBus.emit('TURN:START', { playerIndex: 0, playerName: 'Teste' });
                }
                else {
                    Logger.error(LogCategory.SYSTEM, "❌ EventBus não encontrado!");
                }
        ...
        // DummyBot Button
        const btnDummy = this.container.querySelector("#btn-use-dummybot");
        if (btnDummy)
            btnDummy.onclick = () => {
                if (window.DummyBot) {
                    window.BotBrain = window.DummyBot;
        ...
        const btnClear = this.container.querySelector("#btn-log-clear");
        if (btnClear)
            btnClear.onclick = () => Logger.clear();
        // Bot Profile Selector - REMOVIDO (obsoleto no BotBrain v5.0)
        // Tutorial Automator Button
        const btnTut = this.container.querySelector("#btn-debug-tutorial");
        if (btnTut)
            btnTut.onclick = () => {
                Logger.info(LogCategory.SYSTEM, "Starting Tutorial Automator...");
                if (window.TutorialAutomator) {
                    window.TutorialAutomator.run();
                }
                else {
                    ...
                    script.onload = () => {
                        if (window.TutorialAutomator)
                            ...
                            Logger.error(LogCategory.SYSTEM, "Failed to initialize TutorialAutomator after load.");
                    };
                    document.head.appendChild(script);
                }
            };
        // Bot vs Bot Button
        const btnBvB = this.container.querySelector("#btn-debug-botvsbot");
        if (btnBvB)
            btnBvB.onclick = () => {
                ...
                if (window.PvEAutomator) {
                    ...
                    script.onload = () => {
                        if (window.PvEAutomator)
                            ...
                            Logger.error(LogCategory.SYSTEM, "Failed to initialize PvEAutomator.");
                    };
                    document.head.appendChild(script);
                }
            };
