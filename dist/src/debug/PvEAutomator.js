...
export const PvEAutomator = {
    ...
    loop: function () {
        ...
        if (!estado) {
            if (this.stepCount % 5 === 0)
                Logger.warn(LogCategory.GAME, "[PvEAutomator] Waiting for estadoJogo...");
            return;
        }
    ...
    performRandomAction: function (estado) {
        ...
        estado.mesa.forEach((p, i) => {
            if (p === null)
                validSlots.push({ type: 'empty', index: i });
            else if (p.virada)
                validSlots.push({ type: 'hidden', index: i });
            else
                validSlots.push({ type: 'visible', index: i });
        });
                ...
                case 'hide':
                    ...
                    if (visible) {
                        ...
                        if (window.avancarTurno)
                            window.avancarTurno();
                        if (window.Renderer)
                            window.Renderer.renderizarMesa(); // Force update
                    }
                    break;
                case 'swap':
                    if (validSwapSlots.length < 2)
...
window.PvEAutomator = PvEAutomator;
