...
const InputHandler = {
    ...
    setupGlobalListeners: function () {
        ...
        if (btnCara) {
            btnCara.onclick = () => {
                ...
                if (window.definirEscolha)
                    window.definirEscolha("cara");
            };
        }
    ...
    setupReservaItem: function (element, pedraObj, index, isVertical) {
        ...
        if (!InputHandler.isTouchDevice()) {
            element.onmouseenter = (e) => window.showTooltip("Arraste para o tabuleiro", e.clientX, e.clientY); // Global helper
            element.onmousemove = (e) => window.showTooltip("Arraste para o tabuleiro", e.clientX, e.clientY);
            element.onmouseleave = window.hideTooltip; // Global helper
        }
    },
    handleDragStart: function (e, element, pedraObj, index, isTouch) {
        if (window.tellstonesTutorial) {
            if (!window.tellstonesTutorial.verificarAcao("ARRASTAR_RESERVA"))
                return;
        }
        // Validações de turno
        if (window.ehMinhaVez && !window.ehMinhaVez())
            return; // Global check
        if (window.estadoJogo && !window.estadoJogo.alinhamentoFeito)
            return;
    ...
    showDropZones: function () {
        ...
        const mesa = window.estadoJogo ? window.estadoJogo.mesa : Array(7).fill(null);
        // Assuming GameRules is available globally (it is)
        const validos = window.GameRules ? window.GameRules.calcularSlotsValidos(mesa) : [3];
        ...
        if (window.GameController) {
            ...
            if (sucesso) {
                if (window.showToastInterno)
                    window.showToastInterno("Pedra colocada!");
                if (window.tellstonesTutorial)
                ...
                if (typeof window.setupMesaInteractions === 'function')
                    window.setupMesaInteractions();
            }
        }
        else {
            // Fallback Legado
