...
window.showTooltip = function (text, x, y) {
    ...
    if (tooltip && text) {
        ...
        let finalX = x + 15;
        let finalY = y + 15;
        if (finalX + rw > window.innerWidth)
            finalX = x - rw - 10;
        if (finalY + rh > window.innerHeight)
            finalY = y - rh - 10;
...
const Renderer = {
                ...
                div.ondblclick = (e) => {
                    ...
                    if (p.virada) {
                        ...
                        if (window.getDBRef && window.salaAtual && window.nomeAtual) {
                            ...
                            setTimeout(() => {
                                if (window.getDBRef)
                                    window.getDBRef("salas/" + window.salaAtual + "/estadoJogo/mesaEspiada").remove();
                            }, 2200);
                        }
                        // 5. Notify Tutorial
    ...
    renderizarMarcadoresPonto: function () {
        if (!window.estadoJogo || !window.estadoJogo.jogadores)
            return;
    ...
    _executarAnimacaoAlinhamento: function (circle, wrapper, tabuleiroCenter) {
        ...
        const executeCallback = () => {
            ...
            const pedrasDivs = Array.from(circle.querySelectorAll(".pedra-circulo:not(.pedra-central)"));
    ...
    animarBotColocar: function (pedra, fromIdx, slotIdx, callback) {
        ...
        if (reservaContainer) {
            const pedrasMao = Array.from(reservaContainer.children);
    ...
    renderBotMemoryDebug: function (botBrain) {
        if (window.Logger && window.Logger.game)
            window.Logger.game("[Renderer] renderBotMemoryDebug pipeline started.");
        ...
        if (!isDevMode) {
            if (container)
                container.remove(); // Ensure it's removed if dev mode is off
            return;
        }
        if (!container) {
