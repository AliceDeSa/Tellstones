...
export const CoinFlip = {
    listenerRef: null as any,
    ultimoLadoNotificado: null as string | null,
    ultimoResultadoMoedaTocado: null as any,
    ...
    resetUI: function () {
        ...
        if (moedaBtn) moedaBtn.style.display = "none";
    },

    definirEscolha: function (escolha: string) {
        if (!window.salaAtual || !window.nomeAtual) return;

        window.getDBRef("salas/" + window.salaAtual + "/caraCoroa/escolha").transaction(function (current: any) {
            if (current === null) {
    ...
    ouvirCaraCoroa: function () {
        ...
        this.listenerRef = window.getDBRef("salas/" + window.salaAtual + "/caraCoroa").on("value", (snap: any) => {
            const data = snap.val();
            // window.ultimoCaraCoroaData = data; // Legacy global??

            const escolhaDiv = document.getElementById("escolha-cara-coroa");
            const btnCara = document.getElementById("btn-cara") as HTMLButtonElement | null;
            const btnCoroa = document.getElementById("btn-coroa") as HTMLButtonElement | null;
            ...
            if (minhaEscolha !== this.ultimoLadoNotificado) {
                const text = LocaleManager.t('game.coinFlip.youGot').replace('{side}', minhaEscolha.toUpperCase());
                if (window.notificationManager) window.notificationManager.showInternal(text);
    ...
    mostrarMoedaParaSorteioCriador: function () {
        ...
        window.getDBRef("salas/" + window.salaAtual + "/caraCoroa/resultado").once("value", (snapshot: any) => {
            if (snapshot.exists()) {
            ...
            window.getDBRef("salas/" + window.salaAtual + "/caraCoroa/escolha").once("value", (snapEscolha: any) => {
                if (!snapEscolha.exists()) {
                    if (window.notificationManager) window.notificationManager.showInternal("Aguarde escolha de Cara ou Coroa!");
                    moedaBtn.style.display = "none";
                    return;
                }
                ...
                moedaBtn.onclick = () => {
                    moedaBtn.onclick = null;
                    const moedaAnimada = document.getElementById("moeda-animada") as HTMLElement;
                    if (moedaAnimada) {
                        moedaAnimada.classList.remove("moeda-girando");
                        void moedaAnimada.offsetWidth;
                        moedaAnimada.classList.add("moeda-girando");
                    }
                    window.getDBRef("salas/" + window.salaAtual + "/caraCoroa/resultado").transaction((current: any) => {
                        if (current === null) return Math.random() < 0.5 ? 0 : 1;
                        return;
                    });
                };
            });
    ...
    mostrarMoedaParaSorteioSincronizado: function (resultado: number, minhaEscolha: string) {
        this.garantirBotao();
        const moedaBtn = document.getElementById("moeda-btn");
        ...
        const somMoeda = document.getElementById("som-moeda") as HTMLAudioElement;

        if (!moedaAnimada || !moedaFrente || !moedaVerso || !somMoeda) return;
    ...
    mostrarNotificacaoMoeda: function (msg: string) {
    ...
    tentarDefinirVencedorMoeda: function (resultado: number) {
        ...
        window.getDBRef("salas/" + window.salaAtual + "/caraCoroa/escolha").once("value", (snapEscolha: any) => {
            ...
            let nomeGanhador: string | null = null;
            const jogadores = window.estadoJogo.jogadores || [];

            if (escolhaData && jogadores.length > 0) {
                if (resultado === 0 && escolhaData.escolha === "cara") nomeGanhador = escolhaData.nome;
                else if (resultado === 1 && escolhaData.escolha === "coroa") nomeGanhador = escolhaData.nome;
                else {
                    // O outro ganhou
                    nomeGanhador = jogadores.find((j: any) => j.nome !== escolhaData.nome)?.nome;
                }
            }

            if (nomeGanhador) {
                const idx = jogadores.findIndex((j: any) => j.nome === nomeGanhador);
                if (idx !== -1) {
                    window.getDBRef("salas/" + window.salaAtual + "/estadoJogo").once("value", (snapEstado: any) => {
...
(window as any).CoinFlip = CoinFlip;
