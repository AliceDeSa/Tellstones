...
import { RoomInfo } from '../modes/multiplayer/RoomListManager.js';

export class RoomListScreen {
    private container: HTMLElement | null = null;
    private listContainer: HTMLElement | null = null;
    private refreshBtn: HTMLElement | null = null;
    private tabBtn: HTMLElement | null = null;
    ...
    private registerEventListeners() {
        EventBus.on(EventType.ROOM_LIST_UPDATE, (data: { rooms: RoomInfo[] }) => {
    ...
    private registerDOMEvents() {
        ...
        if (this.refreshBtn) {
            this.refreshBtn.addEventListener('click', () => {
                ...
                if (this.refreshBtn) this.refreshBtn.innerText = "Atualizando...";
                setTimeout(() => { if (this.refreshBtn) this.refreshBtn.innerText = "Atualizar"; }, 1000);
            });
        }
    ...
    private renderList(rooms: RoomInfo[]) {
    ...
    private joinRoom(room: RoomInfo) {
        // Fill the join form and submit
        const tabs = document.getElementById('online-tabs-container');
        if (tabs) {
            const joinBtn = document.getElementById('join-room-btn');
            if (joinBtn) joinBtn.click(); // Switch tab
        }

        const roomInput = document.getElementById('room-code') as HTMLInputElement;
        if (roomInput) roomInput.value = room.code;

        // Auto-focus name input
        const nameInput = document.getElementById('nome-entrar') as HTMLInputElement;
