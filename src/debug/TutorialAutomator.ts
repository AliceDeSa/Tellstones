...
export class TutorialAutomator {
    ...
    private async ensureInTutorial() {
    ...
    private async runTutorialFlow(scenario: Scenario): Promise<boolean> {
        try {
            while (window.tellstonesTutorial && this.isRunning) {
                ...
                if (!window.tellstonesTutorial.roteiro) return true;
    ...
    private async performAction(stepIdx: number, scenario: Scenario) {
        switch (stepIdx) {
            case 0: break; // Welcome
            case 1: // Place Stone
                if (!window.estadoJogo) return;
                const pIdx = window.estadoJogo.reserva.findIndex((p: any) => p !== null);
                if (pIdx > -1) {
            ...
            case 5: // Challenge (Click Button)
                ...
                await this.wait(200);
                const hiddenIdx = window.estadoJogo.mesa.findIndex((p: any) => p && p.virada);
                if (hiddenIdx > -1) {
            ...
            case 6: // Respond Challenge
                ...
                if (desafio && desafio.idxPedra !== undefined) {
                    ...
                    const correctIdx = officialStones.findIndex((p: any) => p.nome === realStone.nome);
    ...
    private reportFailure(stepIdx: number) {
...
// Global Export
(window as any).TutorialAutomator = TutorialAutomator;
