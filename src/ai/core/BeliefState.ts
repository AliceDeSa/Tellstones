...
interface Observation {
    type: 'place' | 'reveal' | 'hide' | 'swap' | 'peek';
    slot?: number;
    stone?: string;
    from?: number;
    to?: number;
...
export class BeliefState {
    // Probabilidades: slots[slot][stone] = probability
    private slots: Map<number, Map<string, number>>;

    // Pedras confirmadas recentemente
    private confirmed: Map<number, { stone: string; turn: number }>;
    ...
    private readonly FORGET_THRESHOLD = 0.05;
     ...
     */
    observePlacement(slot: number, stone: string): void {
     ...
     */
    observeReveal(slot: number, stone: string): void {
     ...
     */
    observeSwap(from: number, to: number): void {
        ...
        if (confirmedFrom) this.confirmed.set(to, confirmedFrom);
        if (confirmedTo) this.confirmed.set(from, confirmedTo);
     ...
     */
    observePeek(slot: number, stone: string): void {
     ...
     */
    getMostLikelyStone(slot: number): { stone: string; probability: number } {
        const probs = this.slots.get(slot);
        if (!probs) return { stone: STONES[0], probability: 0 };
     ...
     */
    getSlotProbabilities(slot: number): Map<string, number> {
     ...
     */
    getKnownSlots(minConfidence: number = 0.7): number[] {
        const known: number[] = [];
     ...
     */
    private setConfirmed(slot: number, stone: string): void {
        const probs = this.slots.get(slot)!;
     ...
     */
    private eliminateStoneFromOtherSlots(stone: string, exceptSlot: number): void {
        for (let slot = 0; slot < 7; slot++) {
     ...
     */
    private normalize(slot: number): void {
        const probs = this.slots.get(slot)!;
...
(window as any).BeliefState = BeliefState;
