...
export interface BotAction {
    type: 'place' | 'flip' | 'swap' | 'peek' | 'challenge' | 'boast';
    target?: number;
...
export class ActionEvaluator {
     ...
     */
    evaluateActions(state: GameState, validActions: BotAction[]): ScoredAction[] {
     ...
     */
    private scoreAction(action: BotAction, state: GameState): number {
     ...
     */
    private getScoreBreakdown(action: BotAction, state: GameState): {
        baseValue: number;
        informationGain: number;
        winProbability: number;
        riskFactor: number;
    ...
    private evaluatePlace(slot: number, state: GameState): any {
        let baseValue = 25;

        // Preferir centro
        if (slot === 3) baseValue += 10;
        if (slot === 2 || slot === 4) baseValue += 5;
        ...
        if (hasLeftNeighbor || hasRightNeighbor) baseValue += 5;
    ...
    private evaluateFlip(slot: number, state: GameState): any {
        const pedra = state.mesa[slot];
        if (!pedra) return { baseValue: 0, informationGain: 0, winProbability: 0, riskFactor: 0 };
    ...
    private evaluateSwap(from: number, to: number, state: GameState): any {
    ...
    private evaluatePeek(slot: number, state: GameState): any {
        const confidence = this.beliefState.getConfidence(slot);
    ...
    private evaluateChallenge(slot: number, state: GameState): any {
        const pedra = state.mesa[slot];
    ...
    private evaluateBoast(state: GameState): any {
        ...
        pedrasViradas.filter(x => x.hidden).forEach(h => {
            if (this.beliefState.getConfidence(h.idx) > 0.85) knownHidden++;
        });
     ...
     */
    getValidActions(state: GameState): BotAction[] {
     ...
     */
    private getAdjacentEmptySlots(mesa: (Pedra | null)[]): number[] {
        const valid: number[] = [];
...
(window as any).ActionEvaluator = ActionEvaluator;
