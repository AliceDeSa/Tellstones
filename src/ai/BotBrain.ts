...
export class BotBrain {
     ...
     */
    observe(action: any, state: GameState): void {
        if (!action) return;
     ...
     */
    updateMemory(slot: number, nome: string, confianca: number): void {
     ...
     */
    private getValidMoves(state: GameState): BotAction[] {
     ...
     */
    private scoreMove(move: BotAction, state: GameState): number {
        switch (move.type) {
            case 'place':
    ...
    private scorePlace(slot: number, state: GameState): number {
        ...
        let score = 30;
        if (slot === 3) score += 10;
        if (slot === 2 || slot === 4) score += 5;
        return score;
    }

    private scoreFlip(slot: number, state: GameState): number {
     ...
     */
    private getVisibleStones(state: GameState): string[] {
        return state.mesa
            .filter(p => p && !p.virada)
            .map(p => p!.nome);
    }
