...
class MentalModel {
    ...
    observePlayerAction(action: any, state: any) {
        const now = Date.now();
...
export class BotBrain {
    ...
    calculateThinkTime(state: any, decision: any) {
        let base = 1500;
    ...
    observe(action: any, state: any) {
        ...
        if (action.tipo === 'turn_end') {
            ...
            if (state && state.mesa) {
                state.mesa.forEach((p: any, i: number) => {
                    if (p && !p.virada) {
    ...
    decayMemory(state: any) {
        let retention = this.profile.memory.retention;
        let hiddenCount = 0;
        if (state && state.mesa) {
            hiddenCount = state.mesa.filter((p: any) => p && p.virada).length;
        }
        else {
    ...
    updateMemory(slot: number, name: string, confidence: number) {
        this.memory[slot] = {
            stoneName: name,
    ...
    getChatter(event: string, context?: any) {
        ...
        if (list && list.length > 0) {
            const available = list.filter((m: string) => m !== this.lastChatter);
    ...
    decideMove(state: any) {
        ...
        const visibleStones = state.mesa.filter((p: any) => p && !p.virada).length;
        const handStones = state.reserva && state.reserva.some((p: any) => p !== null);
        const myScore = ((_a = state.jogadores.find((j: any) => j.id === 'p2')) === null || _a === void 0 ? void 0 : _a.pontos) || 0;
        const oppScore = ((_b = state.jogadores.find((j: any) => j.id === 'p1')) === null || _b === void 0 ? void 0 : _b.pontos) || 0;
    ...
    choosePlaceTarget(state: any) {
        ...
        const validSlots = (window as any).GameRules?.calcularSlotsValidos(state.mesa) || [];
    ...
    chooseSwapTargets(state: any) {
        const indices = state.mesa.map((p: any, i: number) => p ? i : -1).filter((i: number) => i !== -1);
        if (indices.length < 2)
            return null;
        const hiddenCount = state.mesa.filter((p: any) => p && p.virada).length;
        if (hiddenCount === 0)
            return null;
        if (this.mentalModel) {
            ...
            if (knownToPlayer.length > 0) {
                ...
                const others = indices.filter((i: number) => i !== targetA);
                if (others.length > 0) {
        ...
        const visible = indices.filter((i: number) => state.mesa[i] && !state.mesa[i].virada);
        const hidden = indices.filter((i: number) => state.mesa[i] && state.mesa[i].virada);
        let idxA, idxB;
    ...
    chooseFlipTarget(state: any) {
        const visibleIndices = state.mesa.map((p: any, i: number) => (p && !p.virada) ? i : -1).filter((i: number) => i !== -1);
        if (visibleIndices.length === 0)
            return -1;
        return visibleIndices[Math.floor(Math.random() * visibleIndices.length)];
    }
    choosePeekTarget(state: any) {
        const hiddenIndices = state.mesa.map((p: any, i: number) => (p && p.virada) ? i : -1).filter((i: number) => i !== -1);
    ...
    chooseChallengeTarget(state: any) {
        const hiddenIndices = state.mesa.map((p: any, i: number) => (p && p.virada) ? i : -1).filter((i: number) => i !== -1);
        if (hiddenIndices.length === 0)
            return -1;
        const candidates = hiddenIndices.filter((idx: number) => {
            const mem = this.memory[idx];
    ...
    checkSignatureMove(state: any) {
        const profile = this.profile.name;
        if (profile === 'Lógico') {
            const knownCount = Object.values(this.memory).filter((m: any) => m.confidence > 0.9).length;
            if (knownCount >= 4) {
                const unsure = this.findUnsureHiddenStone(state);
                if (unsure !== -1)
        ...
        if (profile === 'Trapaceiro') {
            ...
            if (lastAction && lastAction.tipo === 'trocar') {
                ...
                const others = state.mesa.map((p: any, i: number) => i).filter((i: number) => i !== lastAction.origem && i !== lastAction.destino);
                if (others.length > 0) {
    ...
    findUnsureHiddenStone(state: any) {
        const hiddenIndices = state.mesa.map((p: any, i: number) => (p && p.virada) ? i : -1).filter((i: number) => i !== -1);
        for (let idx of hiddenIndices) {
            const mem = this.memory[idx];
            if (!mem || mem.confidence < (this.config.confidenceToPeek || 0.85))
    ...
    findOldestHidden(indices: number[], state: any) {
    ...
    guessByElimination(targetSlot: number) {
    ...
    calculateAverageHiddenConfidence(state: any) {
        let total = 0;
        let count = 0;
        state.mesa.forEach((p: any, i: number) => {
            if (p && p.virada) {
    ...
    getDebugStats() {
        ...
        const confidences = Object.values(this.memory).map((m: any) => m.confidence.toFixed(2)).join(', ');
...
(window as any).BotBrain = BotBrain;
