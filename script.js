...
function atualizarPlacarETurno() {
  ...
  placarDiv.innerHTML = estadoJogo.jogadores
    .map(
      (j, i) =>
        `<span${
          i === estadoJogo.vez ? ' style="text-decoration:underline;"' : ""
        }>${j.nome}: ${j.pontos}</span>`
    )
    .join(" <b>|</b> ");
  turnoDiv.innerHTML = `Vez de <strong>${
    estadoJogo.jogadores[estadoJogo.vez].nome
  }</strong>`;
}
...
function acaoDesafiar() {
  showToastInterno("Ação: Desafiar (implementar lógica)");
}
function acaoSeGabar() {
  showToastInterno("Ação: Se Gabar (implementar lógica)");
}
  ...
  pedrasReserva.forEach((pedra) => {
    ...
    pedra.addEventListener("mousedown", function (e) {
      ...
      function onUp(ev) {
        ...
        if (
          ev.clientX >= mesaRect.left &&
          ev.clientX <= mesaRect.right &&
          ev.clientY >= mesaRect.top &&
          ev.clientY <= mesaRect.bottom
        ) {
          ...
          const leftFinal =
            mesaRect.left +
            (pos === 0 ? slotWidth / 2 : mesa.offsetWidth - slotWidth / 2) -
            larguraPedra / 2;
          const topFinal =
            mesaRect.top + mesa.offsetHeight / 2 - larguraPedra / 2;
          ghost.animate(
            [
    ...
    central.onmousedown = function (e) {
      ...
      function onUp(ev) {
        ...
        if (
          ...
          estadoJogo.mesa = [
            ...estadoJogo.mesa,
            { ...estadoJogo.pedraCentral, virada: false }
          ];
...
function setupDragDropVertical() {
  const pedrasVerticais = document.querySelectorAll(
    "#circle-pedras .pedra-circulo:not(.pedra-central)"
  );
    ...
    div.onmousedown = function (e) {
      ...
      function onUp(ev) {
        ...
        if (
          ...
          const pedrasRestantes = document.querySelectorAll(
            "#circle-pedras .pedra-circulo:not(.pedra-central)"
          );
...
function animarPedraParaMesa(pedra, slotIndex) {
  ...
  // Posição final
  const leftFinal =
    slotLargura * slotIndex + slotLargura / 2 - larguraPedra / 2;
...
function renderizarPedrasMesa(pedras) {
  ...
  pedras.forEach((p, i) => {
    if (p && p.url) {
      // Só cria a div se houver pedra válida
      const div = document.createElement("div");
...
function mostrarMoedaParaSorteio() {
  ...
  moedaBtn.onclick = function () {
    ...
    setTimeout(() => {
      ...
      // Exibe o resultado
      if (
        (resultado === 0 && escolhaJogador === "cara") ||
        (resultado === 1 && escolhaJogador === "coroa")
      ) {
          ...
          setTimeout(() => {
            ...
            if (centralDiv) {
              ...
              if (img) {
                ...
                const tabuleiroCenter = document.getElementById(
                  "tabuleiro-center"
                );
                ...
                const leftFinal =
                  wrapperRect.left - centerRect.left + wrapper.offsetWidth / 2;
                const topFinal =
                  wrapperRect.top - centerRect.top + wrapper.offsetHeight / 2;
                // Cria pedra animada centralizada
                const pedraAnimada = document.createElement("div");
                pedraAnimada.className =
                  "pedra-mesa pedra-oficial pedra-animada-mesa";
